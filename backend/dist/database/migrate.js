...
async function createSchema() {
    try {
        logger_1.default.info('Starting database schema creation...');
        await createEnums();
        await createTables();
        await createIndexes();
        await insertInitialData();
        logger_1.default.info('Database schema creation completed successfully');
    }
    catch (error) {
        logger_1.default.error('Database schema creation failed', error);
        throw error;
    }
}
async function createEnums() {
    ...
    for (const enumQuery of enums) {
        try {
            ...
            logger_1.default.info(`Created enum: ${enumQuery.split('AS ENUM')[0].split('CREATE TYPE')[1].trim()}`);
        }
        catch (error) {
            if (error.code !== '42710') {
                throw error;
...
async function createTables() {
    ...
    for (const tableQuery of tables) {
        ...
        logger_1.default.info(`Created table: ${tableName}`);
    }
}
async function createIndexes() {
    ...
    for (const indexQuery of indexes) {
        ...
        logger_1.default.info(`Created index: ${indexName}`);
    }
}
async function insertInitialData() {
    ...
    try {
        ...
        logger_1.default.info('Created super admin user');
    }
    catch (error) {
        if (error.code !== '23505') {
            throw error;
